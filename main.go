...
func startSNIproxy() {
	...
	defer cancel()
	serviceLogger(fmt.Sprintf("Starting SNI Proxy on port %v", cfg.SNIPort), 0)
	listener, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.SNIPort))
	if err != nil {
	...
	go func(listener net.Listener) {
		...
		for {
			...
			serviceLogger(fmt.Sprintf("Connection From %s", raddr.String()), 32)
			go serve(connection, raddr.String())
		}
	}(listener)
